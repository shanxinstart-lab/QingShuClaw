...
test('formatTokenCount returns values below 1000 as-is', () => {
...
test('formatMessageTime shows only time for today', () => {
  const now = new Date();
...
test('formatMessageTime shows month and day for earlier this year', () => {
  vi.useFakeTimers();
...
test('formatMessageTime shows full date for other years', () => {
  vi.useFakeTimers();
  vi.setSystemTime(new Date(2026, 4, 7, 10, 0));
...
test('formatMessageDateTime always shows full date and time', () => {
  vi.useFakeTimers();
  vi.setSystemTime(new Date(2026, 4, 7, 10, 0));
