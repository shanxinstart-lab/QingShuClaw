...
function normalizePopoEnterpriseConfig(cfg: unknown): unknown {
  ...
  const firstAccount = accountEntries[0]?.[1];
  if (!firstAccount) {
    return cfg;
  }

  if (accountEntries.length > 1) {
    console.warn('[Enterprise] POPO enterprise config contains multiple accounts; applying the first account in single-instance mode');
  }
  return firstAccount;
}

type OptionalPopoMultiInstanceStore = {
  setPopoMultiInstanceConfig?: (config: { instances: Array<Record<string, unknown>> }) => void;
  setPopoConfig: (config: unknown) => void;
};

function syncPopoEnterpriseConfig(imStore: IMStore, cfg: unknown): void {
  const normalizedCfg = normalizeMultiAccountChannelConfig('moltbot-popo', cfg);
  const accounts = readAccountsFromChannelConfig(normalizedCfg);
  const popoStore = imStore as unknown as OptionalPopoMultiInstanceStore;

  if (accounts && typeof popoStore.setPopoMultiInstanceConfig === 'function') {
    const instances = Object.entries(accounts).map(([accountId, accountCfg], idx) => ({
      ...accountCfg,
      instanceId: accountId,
      instanceName: typeof accountCfg.name === 'string' ? accountCfg.name : `POPO Bot ${idx + 1}`,
    }));
    popoStore.setPopoMultiInstanceConfig({ instances });
    return;
  }

  popoStore.setPopoConfig(normalizePopoEnterpriseConfig(cfg));
}
...
function syncIMChannels(configPath: string, imStore: IMStore): void {
  ...
  try {
    ...
    // For multi-instance platforms, update instances when enterprise config
    // supplies account maps instead of replacing the whole platform config.
    const PLATFORM_SETTERS: Record<string, (cfg: any) => void> = {
      ...
      'moltbot-popo': (cfg) => syncPopoEnterpriseConfig(imStore, cfg),
...
function syncCoworkConfig(
  ...
  try {
    ...
    const importedWorkingDirectory = agents?.defaults?.cwd || agents?.defaults?.workspace;
    if (importedWorkingDirectory) {
      updates.workingDirectory = importedWorkingDirectory;
    }
