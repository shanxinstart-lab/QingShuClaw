...
export function migrateMainAgentWorkspace(
  ...
  const results: CopyResult[] = [];
  const memoryResult = mergeDirIfNeeded(path.join(oldDir, 'memory'), path.join(newDir, 'memory'));
  ...
  for (const filename of BOOTSTRAP_FILES) {
    const result = copyIfNeeded(path.join(oldDir, filename), path.join(newDir, filename));
    results.push(result);
    if (result.changed) {
